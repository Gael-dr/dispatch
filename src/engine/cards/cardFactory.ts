import type { Card, CardTypeId, CardStatus } from './card.types'
...
export class CardFactory {
    ...
    register<TPayload>(bp: CardBlueprint<TPayload>) {
        ...
        this.blueprints.set(bp.type, bp)
        return this
    }

    create<TPayload = unknown>(
        type: CardTypeId,
        overrides?: Partial<Omit<Card<TPayload>, 'type' | 'payload'>> & { payload?: TPayload },
        seed = Date.now(),
    ): Card<TPayload> {
        ...
        const status: CardStatus = (overrides?.status as CardStatus | undefined) ?? 'pending'
    ...
    createMany(type: CardTypeId, count: number, seed = Date.now()) {
        return Array.from({ length: count }, (_, i) => this.create(type, undefined, seed + i))
    }
