...
export default function SettingsView() {
  ...
  const isEnabled = (cardType: CardTypeId, connectorId: string) =>
    settings[cardType]?.enabledConnectors.includes(connectorId as ConnectorId)

  const toggleConnector = async (connectorId: ConnectorId) => {
    if (!selectedType) return
    const currentlyEnabled =
      settings[selectedType].enabledConnectors.includes(connectorId)
    ...
    if (currentlyEnabled)
      await disconnectProviderOnBackend(selectedType, connectorId)
  ...
  return (
    ...
    >
      ...
      <section className="max-w-3xl">
        <p className="text-slate-400 text-xs font-bold tracking-wide mb-3">
          TYPES DE CARTES
        </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {CARD_TYPES.map(type => {
            ...
            const enabledCount =
              settings[type.id]?.enabledConnectors.length ?? 0
        ...
        {loading && (
          <p className="text-slate-400 text-sm font-medium mt-4">Chargement…</p>
        )}
      </section>
